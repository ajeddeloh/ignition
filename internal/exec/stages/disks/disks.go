...
import (
	...
	putil "github.com/coreos/ignition/config/util"
...
func partitionMatches(existing, spec types.Partition) error {
	if spec.Number != existing.Number {
		// sanity check
		return fmt.Errorf("partition numbers did not match (specified %d, got %d). This should not happen.", spec.Number, existing.Number)
	}
